...
## Shipped MVP: Cable CCC (AS/NZS 3008)

`cable-ccc-asnzs` — fail-closed lookup of tabulated current-carrying capacity. See `docs/ASNZS_CABLE_CCC.md`.

- AS/NZS 3000 is the installation / selection obligation; **AS/NZS 3008.1.1** holds the CCC tables.
- Shipped without ampacity numbers (copyright). Load licensed cells with `tableId` / `columnId`.
- Distinct from the IEC voltage-drop calculator (resistivity + optional reactance only) and from the adiabatic short-circuit CSA tool. Voltage drop has a soft CTA into this lookup.

## Still deferred from this CCC MVP

- Fuller AS/NZS 3008 derating matrix (ambient, grouping, soil, burial depth) once factor tables can be loaded with citations
- AS/NZS 3008.1.2 (typical New Zealand conditions)
- AS/NZS 3008.1.1:2025 table identifiers / DC appendix
- Single-core, buried, V-75, and other installation columns
- Full AS/NZS 3000 voltage-drop / earth-loop / max-demand suite
...
- Cable CCC (AS/NZS 3008) — `cable-ccc-asnzs` (MVP lookup; licensed tables not embedded)
